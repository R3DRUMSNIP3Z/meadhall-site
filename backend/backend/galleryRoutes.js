// backend/galleryRoutes.js — Cloudinary (if configured) + disk fallback
...
function install(app) {
  ...
  const acceptMany = (field) => (req, res, next) =>
    galleryMulter.array(field, 20)(req, res, (err) => (err && err.message !== "Unexpected field") ? next(err) : next());

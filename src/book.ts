...
function getMeta(name: string): string {
  return (document.querySelector(`meta[name="${name}"]`) as HTMLMetaElement)?.content?.trim() || "";
}
...
const SITE_BASE = location.origin;                                     // the page origin (Vercel)
const API_BASE  = getMeta("api-base") || SITE_BASE;                    // backend (Render) unless overridden
const ASSET_BASE = withTrailingSlash(getMeta("asset-base") || "/guildbook/"); // where images live on Vercel

/* ---------------- story URL resolution ---------------- */

/**
 * Resolve the story URL from:
 * 1) <meta name="book-src" content="/guildbook/file.rtf">  (site-relative → Vercel)
 * 2) <meta name="book-src" content="https://...">           (absolute)
 * 3) OR ?book=name → API_BASE/uploads/name.(rtf|txt)        (Render fallback)
 */
function resolveStoryUrl(): string {
  const meta = getMeta("book-src");
  if (meta) {
    if (/^https?:\/\//i.test(meta)) return meta;                 // absolute URL
    if (meta.startsWith("/")) return SITE_BASE + meta;           // site-relative → Vercel
    return `${SITE_BASE}/${meta.replace(/^\.?\//, "")}`;         // relative → Vercel
  }

  // query fallback: ?book=samplebook  (probe Render uploads)
  ...
  return `${tryRtf}||${tryTxt}`; // special “two-choice” URL handled below
}
...
/**
 * Map any old Render upload paths to the Vercel /guildbook/ folder, and
 * also make bare filenames resolve under ASSET_BASE.
 */
function assetUrl(u: string): string {
  let s = (u || "").trim();
  if (!s) return s;

  // Absolute Render → Vercel /guildbook/
  const renderUploadsAbs = /^https?:\/\/meadhall-site\.onrender\.com\/uploads\/(?:samplebook|guildbook)\//i;
  if (renderUploadsAbs.test(s)) {
    s = s.replace(renderUploadsAbs, ASSET_BASE);
    if (!s.startsWith("http") && !s.startsWith("/")) s = "/" + s;
    return s;
  }

  // Relative Render path → Vercel /guildbook/
  s = s.replace(/^\/?uploads\/(?:samplebook|guildbook)\//i, ASSET_BASE);

  // Bare filename → under ASSET_BASE
...
function extractFrontMatter(src: string): FrontMatter {
  ...
  if (cover || title || subtitle || credit) {
    const fullCover = cover ? assetUrl(cover) : "";
    coverHtml = `
      <section class="cover" style="min-height:60vh;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center">
        ${fullCover ? `<img src="${fullCover}" alt="Cover" style="max-width:70%;height:auto;border:1px solid rgba(200,169,107,.45);border-radius:14px;margin:10px auto 18px;display:block"/>` : ""}

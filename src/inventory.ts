...
function toast(msg: string) {
  ...
  Object.assign(t.style, {
    ...
    padding:'8px 12px', borderRadius:'10px', zIndex:'100000',
...
function ensureUI() {
  ...
  if (!document.getElementById("vaBagBtn")) {
    ...
    btn.id = "vaBagBtn";
    btn.title = "Inventory";
    btn.innerHTML = `<img src="/guildbook/ui/inventorybag.png" alt="Bag" style="width:54px;height:54px;object-fit:contain;display:block">` +
  ...
  if (!document.getElementById("vaInvOverlay")) {
    ...
    ov.id = "vaInvOverlay";
    ov.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.55);display:none;align-items:center;justify-content:center;z-index:100000;";
    ov.innerHTML = `
...
function open() {
  ...
  setBadge(items);
  renderGrid(items);
  (document.getElementById("vaInvOverlay") as HTMLDivElement).style.display = "flex";
}
function close() {
  (document.getElementById("vaInvOverlay") as HTMLDivElement).style.display = "none";
}
function toggle() {
